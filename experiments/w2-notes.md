...
## Step 14: 「VM のクロックが壊れている」の半分は NTP の選び方だった

> この節は当初「誤診だった。壊れていたのは NTP の選び方」という題で、
> 仮想化の問題を否定する結論になっていた。**Step 16 で訂正済み。**
> 時刻源の問題は実在して直ったが、それとは別に vz のクロック飢餓も実在する。
...
- 判断・回避策:
  - **Step 13 で書いた「VM のクロック規律が劣化」「vz のクロック取りこぼし」は誤診。**
    ...
    **RTT 257ms・ジッタ 30ms のソースから chrony が周波数を推定しようがなかった**結果。
    ...
    > **Step 16 で訂正。** この「誤診だった」という判定自体が過剰な訂正だった。
    > NTP 源の問題は確かに実在して直ったが、**vz のクロック飢餓も別に実在する**。
...
> **Step 16 で訂正。** この節の結論（「LAN の片道遅延は NTP の精度の外側」）は早すぎた。
> 下の測定は **chrony 再起動の数分後・収束前の1回**で、代表性が無い。
> 一晩の実データでは**両方向とも正の値でゲートを通って蓄積されている**（0.01〜2.27 ms）。
> ただし RTT 0.95ms に対して 5 倍の幅で振れており、まだネットワーク遅延ではなく
> 相対クロック誤差を測っている状態。詳細は Step 16。
...
- **「VM のクロックが不安定」だと思ったら、まず NTP の選び方を見る**という流れ。
  仮想化のせいにしたくなる症状（Frequency 2586ppm、falseticker 連発、Skew 悪化）の
  大半は、実は**地球の反対側の NTP サーバを見ていただけ**だった。ディストリの既定値は
  「そのディストリの利用者全体にとっての無難」であって、自分の場所にとっての最適ではない。
  （ただしこれで全部が説明できたわけではない。Step 16 参照）
- **自己申告の誤差は信用できない、を実データで示せる。** max-clock-error 0.19ms と
  申告しながら、実際の相対オフセットはそれより大きい。だから値そのものの妥当性検査が要る。
...
## Step 16: Step 14 の訂正 — 仮想化のクロック飢餓は実在した。深夜だけ

- 日付: 2026-07-30
- やったこと: ダッシュボードで `clock_error` に 300ms 級のスパイクが見えたので追った。
  Step 14 では「VM のクロックが構造的に不安定」を誤診と判定していたので、それとの矛盾を確認した。
- 結果 / エラー:
  - **時間帯できれいに分かれていた。**
  ...
  - 深夜のログ:
    ...
    03:05:05  System clock was stepped by 2.167282 seconds
    03:05:23  System clock was stepped by 2.293691 seconds
       （15〜20秒おきに、7時間で270回）
    ```

    chrony が秒単位のズレを補正し、20秒後にまた秒単位ズレる、を一晩繰り返している。
  - **原因は VM 側で確定。**
    - RasPi は同時刻帯に `systemd-timesyncd` のログが**1件も無い**（`-- No entries --`）
    - Mac は**スリープしていない**（`pmset -g log` に 00:00〜07:00 のスリープ/ウェイク無し）
    - つまりスリープではなく、ホスト側のスケジューリングで vz の VM のクロックが飢餓状態になる
  - **5.79 秒のジャンプは NTP 源の品質では説明できない。** Step 14 で直したのは別の問題。
- 判断・回避策:
  - **Step 14 の「誤診だった」という判定自体が過剰な訂正だった。** 正しくは
    **2つの独立した問題が重なっていた**:

    | | 症状 | 原因 | 状態 |
    |---|---|---|---|
    | 問題1 | 終日 `clock_error` 20ms 前後、root dispersion 37.7ms | 遠距離 NTP 源（RTT 257ms） | **解決**（Step 14） |
    | 問題2 | 深夜のみ 0.6〜5.8 秒のステップ補正、`clock_error` 1156ms | vz のクロック飢餓 | **未解決** |

    日中のデータだけを見て問題1の解決を問題2の否定に使ってしまった。
    **片方の原因を潰したことは、もう片方が存在しないことの証拠にならない。**
  - **品質ゲートは正しく機能している。** 24時間で40%が棄却されており、
    壊れた値がグラフに出ているわけではない。ダッシュボードで見える 300ms は
    「棄却の理由を追うために残している `clock_error` の系列」であって、片道遅延ではない。
  - **bare metal 化の検討に実測の根拠が付いた。** 一晩270回・最大5.79秒のステップは
    片道遅延測定にとって致命的で、物理マシンなら起きない（RasPi が実証している）。
    ただし先に**深夜に Mac 側で何が走っているか**を特定する。Time Machine や
    インデックス再構築のような特定可能なものならスケジュールをずらすだけで済む。

### 片道遅延は「測れている」が、まだネットワークを測っていない

Step 14 の「LAN の片道遅延は NTP の精度の外側」という結論も早すぎた。
あれは chrony 再起動の数分後・収束前の1回の測定（-0.26ms）で判断していた。
一晩の実データでは**両方向とも正の値でゲートを通って蓄積されている**:

```
101→104:  1.46 → 2.06（20分で漸増）→ 1.51 → 2.10 → 0.59 → 0.37 → 0.23 …
104→101:  1.19 → 1.20 → 0.55 → 1.60 → 0.80 → 1.38 → 2.08 → 0.55 → 0.13 …
```

ただし RTT が 0.95ms なので片道は 0.47ms 前後でほぼ一定になるべきところ、
実測は **0.01〜2.27ms** と5倍の幅で、**「ゆっくり上昇 → 急に落ちる」鋸歯状**。
これは相対クロックがドリフトして周期的に補正される波形で、
**まだネットワーク遅延ではなく相対クロック誤差を測っている。**
...
RasPi は `systemd-timesyncd` がサーバ1台を34分間隔で引いているだけで、
その間クロックは自由にドリフトする。鋸歯の振幅 1.5〜2ms はここから出ている可能性が高い。
RasPi にも chrony を入れて両端を対称にするのが次の一手（W3 の課題）。

### 記事ネタ

- **「片方の原因を潰したことは、もう片方が無いことの証拠にならない」**という失敗。
  問題1（NTP 源）を直して日中が綺麗になったので、問題2（vz のクロック飢餓）を
  「誤診だった」と切り捨ててしまった。訂正の勢いで訂正しすぎる、という失敗の型。
- **時間帯で切って初めて見えた。** 24時間の中央値は 0.47ms で健全に見える。
  一方 40% は 10ms を超えている。**中央値だけ見ると気づけない**種類の劣化。
- **仮想マシンのクロックは「日中は物理より良く、深夜は3桁悪い」**という非直感的な結果。
  平均で語れない。
- ダッシュボードに「棄却の理由」を残しておいたおかげで気づけた、という設計判断の回収。
  片道遅延だけを出していたら「たまに欠測する」で終わっていた。

## Step 17: 犯人は Lima 自身だった。ホストが毎分ゲストのクロックを上書きしている

- 日付: 2026-07-30
- やったこと: Step 16 で「深夜 03時台・05時台だけ」と切り分けたので、ホスト側で
  その時間に何が走っているかを探した。Mac の unified log とプロセス分布を見ても
  決定的なものが出なかったため、**Lima のホストエージェントのログ**
  （`~/.lima/perfsonar-vm/ha.stderr.log`）を見た。
- 結果 / エラー:
  - **これが答えだった。** Lima のホストエージェントは**10秒ごとにゲストのクロックを監視し、
    ...
    03:01:12  Time sync: guest clock adjusted (was 3392ms off)
    ```

  - ゲスト側の journal にはこう出る。**時刻が巻き戻っている。**
  ...
  - **Step 16 の「深夜 03時台・05時台だけ」も誤りだった。** ゲストの journal に
    `SyncTime` が出るのは大きなドリフトのときだけで、ホスト側のログを見ると
    **VM 作成（7/26）以来ずっと続いている**:

    | | |
    |---|---|
    | 監視ポーリング | **27,452 回**（10秒間隔） |
    | 実際に上書きした回数 | **4,734 回**（ポーリングの約15%） |
  ...
  - chrony は `makestep 0.1 -1` なので、100ms 未満はスルー（slew）、それ以上でステップする。
    **03時台・05時台に「ステップ補正270回」が見えたのは、その時間だけドリフトが
    秒単位に膨らんで chrony 側もステップに踏み切ったから。**
    小さい上書きは一日中ずっと起きていて、chrony は常に負け続けている。
- 判断・回避策:
  - **Lima 2.2.0 にこの機能を無効化する設定は無い。** `lima-guestagent daemon` の
    フラグにも（`--tick` / `--runtime-dir` / `--socket-owner` / `--virtio-port` /
    `--vsock-port` のみ）、デフォルトテンプレートの設定項目にも存在しない。
    `--plain` はマウント・ポートフォワード・containerd ごと落とすので代替にならない。
  - **これは Lima の設計意図**（ゲストの時刻をホストに追従させる）であって、バグではない。
    しかし**自前の NTP 規律と正直な誤差申告が要る測定ノードとは根本的に両立しない。**
  - **片道遅延（0.5ms オーダー）を測る用途では、この VM は使えない。**
    測りたい量が 0.5ms なのに、クロックが68秒ごとに中央値 123ms 上書きされている。
    片道遅延の実測が RTT 0.95ms に対し 0.01〜2.27ms の鋸歯になるのはこれが原因。
  - **RTT / ロス / スループットは無影響**（クロック非依存）。W2 の成果と
    netem 実験の結論はそのまま有効。影響を受けるのは片道遅延だけ。
  - Lima の drift 測定はゲストエージェントとの RPC 往復で計るため、ゲストが忙しいと
    **往復遅延がそのまま「ドリフト」として計上される**可能性がある。その場合 Lima は
    ドリフトを補正しているのではなく**誤差を注入している**ことになる。どちらにしても
    測定ノードとしては使えない。

### W1 の観察の答え合わせ

W1 で「ドリフトが断続的なステップ補正である」と観察していた（experiments/w1-notes.md:71）。
**あれはこれだった。** 当時は chrony かカーネルの挙動だと思っていたが、実体は
Lima のホストエージェントによる強制上書き。

### 設計判断の回収

W2 Step 0 で「LAN 区間の RTT は `latency`(twamp) ではなく **`rtt` テストを `--tool twping`
で実行**する」と決めていた（PROJECT.md の設計判断）。
**この判断のおかげでパイプラインは片道遅延に依存していない。**
今回の発見でも W2 の成果は無傷で、片道遅延は「おまけ」の位置に留まっている。

### 記事ネタ

- **一番効くのがこれ。** 「仮想マシンのクロックは不安定」という一般論ではなく、
  **Lima が毎分ゲストのクロックを上書きしていて、しかも無効化する設定が無い**という
  具体的な機構。しかも**設計意図どおりの正しい動作**。
  「開発用 VM に良いこと」と「測定ノードに良いこと」が正面から衝突する例。
- **3回訂正している。** ①VM のクロックが壊れている → ②いや NTP 源の問題だった →
  ③いや深夜だけ仮想化の問題もある → ④いや一日中で、原因は Lima の時刻同期。
  ログを1段深く（ゲスト → ホストエージェント）見るまで真因に届かなかった。
  **「自分の層のログ」で止まると誤診する**という教訓。
- **品質ゲートが4日間ずっと仕事をしていた。** 中央値 123ms の上書きが68秒ごとに
  起きている環境で、片道遅延メトリクスは壊れた値を出していない。捨てていた。
- 対比として **RasPi（物理）は同期間に一度もこの種の問題を出していない。**
