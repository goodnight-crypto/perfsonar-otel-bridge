...
`2f0fd63`（2026-07-27）から動いており、**7/29 の時点で 100 発の系列は存在していた。**
...
`docs/runbook-w3-screenshots.md` の記述はそのままで足りるが、優先度が上がった。
...
記事に使うショットでは、ブラウザのクロームを含めないか、公開前にマスクする。
PROJECT.md の公開チェックリストに引っかかる項目なので、Phase 5 の整理で TODO を立てる。
