...
**「VM のクロックが構造的に不安定」という当初の見立ては誤りだった。**
`Frequency` が 590〜2586 ppm の幅で振れていたのは vz のクロック取りこぼしではなく、
**RTT 257ms・ジッタ 30ms のソースを食わされた chrony が周波数を推定できていなかった**だけ。
まともな時刻源を与えたら 2.39 ppm に収まった。ハードウェアの問題に見えたものが設定の問題だった。

`max-clock-error` はおおよそ `root dispersion + root delay/2` なので、
0.149 + 4.73 ≒ **4.9ms**。ゲート（10ms）に対して十分な余裕がある。

## さらに詰めるなら

残る支配項は `root delay / 2 ≒ 4.7ms` で、これは NICT までのネットワーク経路そのもの。
これ以上を求めるなら **LAN 内に NTP サーバを置く**のが効く。RasPi は
RootDispersion 1.02ms と健全なので、そこに chrony をサーバとして立てて VM から引けば
root delay は 1ms 未満になり、`max-clock-error` は 1〜2ms 台に入る。

ただし RasPi は測定の対向エンドポイントでもあるため、時刻の基準と測定対象を兼ねることになる。
片道遅延にとって重要なのは両端の**相対**オフセットなので実用上はむしろ有利だが、
絶対時刻の確からしさは下がる。W2 のスコープ外。
