...
負の片道遅延は物理的にありえない。上限 50ms は home-lab-mesh.json の LAN ペア
（RTT 約0.9ms）を前提とした値で、WAN 区間で `latency` テストを使う場合は経路に応じて変える必要がある。

**実データ141 run での効果**（LAN の片道遅延が妥当な範囲を 0〜20ms として評価）:
